...
// use cookie to make a polyfill
        ...
        this.uploader = function(op) {
            ...
            var getUpToken = function(file) {
                if (op.uptoken) {
                    ...
                    // 测试token
                    // ajax.open('GET', 'http://newcms.diyidan.net/api/v1/kol/token', false);
                    ...
                    //         that.token = res.data.qiniuToken;
                    //     }
                    // };
                    ajax.withCredentials=true;
                    ajax.send();
                    if (ajax.status === 200) {
                        ...
                        logger.debug("get new uptoken: ", res.data.qiniuToken);
                    } else {
                    ...
                    if (file) {
                        switch (err.code) {
                            ...
                            case plupload.FILE_SIZE_ERROR:
                                ...
                                errTip = '弹娘温馨提示：浏览器最大只能上传' + max_file_size + '以内的文件喔，请大大重新选择要上传的文件(*･ω< )~ ';
                                break;
    ...
    var QiniuMidea = new QiniuJsSDK();
    global.Qiniu = Qiniu;
    global.QiniuMidea = QiniuMidea
